...
class Camera implements CameraCaptureCallback.CameraCaptureStateListener, ImageReader.OnImageAvailableListener {
    ...
    public Camera(final Activity activity, final SurfaceTextureEntry flutterTexture, final CameraFeatureFactory cameraFeatureFactory, final DartMessenger dartMessenger, final CameraProperties cameraProperties, final ResolutionPreset resolutionPreset, final boolean enableAudio) {
    ...
    private void prepareMediaRecorder(String outputFilePath) throws IOException {
        ...
        final PlatformChannel.DeviceOrientation lockedOrientation = cameraFeatures.getSensorOrientation().getLockedCaptureOrientation();
        ...
        mediaRecorder = mediaRecorderBuilder.setEnableAudio(enableAudio).setMediaOrientation(lockedOrientation == null ? getDeviceOrientationManager().getVideoOrientation() : getDeviceOrientationManager().getVideoOrientation(lockedOrientation)).build();
    }
    ...
    public void open(String imageFormatGroup) throws CameraAccessException {
        ...
        if (!resolutionFeature.checkIsSupported()) {
            ...
            dartMessenger.sendCameraErrorEvent("Camera with name \"" + cameraProperties.getCameraName() + "\" is not supported by this plugin.");
            return;
        }

        // Always capture using JPEG format.
        pictureImageReader = ImageReader.newInstance(resolutionFeature.getCaptureSize().getWidth(), resolutionFeature.getCaptureSize().getHeight(), ImageFormat.JPEG, 1);
        ...
        imageStreamReader = new ImageStreamReader(resolutionFeature.getPreviewSize().getWidth(), resolutionFeature.getPreviewSize().getHeight(), imageFormat, 1);
            ...
            public void onOpened(@NonNull CameraDevice device) {
                ...
                try {
                    ...
                    if (!recordingVideo) { // only send initialization if we werent already recording and switching cameras
                        dartMessenger.sendCameraInitializedEvent(resolutionFeature.getPreviewSize().getWidth(), resolutionFeature.getPreviewSize().getHeight(), cameraFeatures.getExposureLock().getValue(), cameraFeatures.getAutoFocus().getValue(), cameraFeatures.getExposurePoint().checkIsSupported(), cameraFeatures.getFocusPoint().checkIsSupported());
                    }
                } catch (Exception e) {
                    if (BuildConfig.DEBUG) {
    ...
    private void createCaptureSessionWithSessionConfig(List<OutputConfiguration> outputConfigs, CameraCaptureSession.StateCallback callback) throws CameraAccessException {
        cameraDevice.createCaptureSession(new SessionConfiguration(SessionConfiguration.SESSION_REGULAR, outputConfigs, Executors.newSingleThreadExecutor(), callback));
    }
    ...
    private void startCapture(boolean record, boolean stream) throws CameraAccessException {
        ...
        createCaptureSession(CameraDevice.TEMPLATE_RECORD, successCallback, surfaces.toArray(new Surface[0]));
    }
    ...
    private void runPrecaptureSequence() {
        ...
        try {
            ...
            refreshPreviewCaptureSession(null, (code, message) -> dartMessenger.error(flutterResult, "cameraAccess", message, null));
    ...
    private void takePictureAfterPrecapture() {
        ...
        // Orientation.
        final PlatformChannel.DeviceOrientation lockedOrientation = cameraFeatures.getSensorOrientation().getLockedCaptureOrientation();
        stillBuilder.set(CaptureRequest.JPEG_ORIENTATION, lockedOrientation == null ? getDeviceOrientationManager().getPhotoOrientation() : getDeviceOrientationManager().getPhotoOrientation(lockedOrientation));
    ...
    void unlockAutoFocus() {
        ...
        refreshPreviewCaptureSession(null, (errorCode, errorMessage) -> dartMessenger.error(flutterResult, errorCode, errorMessage, null));
    }

    public void startVideoRecording(@NonNull Result result, @Nullable EventChannel imageStreamChannel) {
    ...
    public void setFlashMode(@NonNull final Result result, @NonNull FlashMode newMode) {
        ...
        refreshPreviewCaptureSession(() -> result.success(null), (code, message) -> result.error("setFlashModeFailed", "Could not set flash mode.", null));
    }
    ...
    public void setExposureMode(@NonNull final Result result, @NonNull ExposureMode newMode) {
        ...
        refreshPreviewCaptureSession(() -> result.success(null), (code, message) -> result.error("setExposureModeFailed", "Could not set exposure mode.", null));
    }
    ...
    public void setExposurePoint(@NonNull final Result result, @Nullable Point point) {
        ...
        refreshPreviewCaptureSession(() -> result.success(null), (code, message) -> result.error("setExposurePointFailed", "Could not set exposure point.", null));
    }
    ...
    public void setFocusPoint(@NonNull final Result result, @Nullable Point point) {
        ...
        refreshPreviewCaptureSession(() -> result.success(null), (code, message) -> result.error("setFocusPointFailed", "Could not set focus point.", null));
    ...
    public void setExposureOffset(@NonNull final Result result, double offset) {
        ...
        refreshPreviewCaptureSession(() -> result.success(exposureOffsetFeature.getValue()), (code, message) -> result.error("setExposureOffsetFailed", "Could not set exposure offset.", null));
    }
    ...
    public void setZoomLevel(@NonNull final Result result, float zoom) throws CameraAccessException {
        ...
        if (zoom > maxZoom || zoom < minZoom) {
            String errorMessage = String.format(Locale.ENGLISH, "Zoom level out of bounds (zoom level should be between %f and %f).", minZoom, maxZoom);
        ...
        refreshPreviewCaptureSession(() -> result.success(null), (code, message) -> result.error("setZoomLevelFailed", "Could not set zoom level.", null));
    }
    ...
    private void startPreviewWithVideoRendererStream() throws CameraAccessException, InterruptedException {
        ...
        if (orientationManager != null) {
            rotation = lockedOrientation == null ? orientationManager.getVideoOrientation() : orientationManager.getVideoOrientation(lockedOrientation);
        }
    ...
    private void prepareVideoRenderer() {
        ...
        Thread.UncaughtExceptionHandler videoRendererUncaughtExceptionHandler = new Thread.UncaughtExceptionHandler() {
            ...
            public void uncaughtException(Thread thread, Throwable ex) {
                dartMessenger.sendCameraErrorEvent("Failed to process frames after camera was flipped.");
            }
        };

        videoRenderer = new VideoRenderer(mediaRecorder.getSurface(), resolutionFeature.getCaptureSize().getWidth(), resolutionFeature.getCaptureSize().getHeight(), videoRendererUncaughtExceptionHandler);
    }
    ...
    public void setDescriptionWhileRecording(@NonNull final Result result, CameraProperties properties) {
        ...
        cameraFeatures = CameraFeatures.init(cameraFeatureFactory, cameraProperties, activity, dartMessenger, resolutionPreset);
        cameraFeatures.setAutoFocus(cameraFeatureFactory.createAutoFocusFeature(cameraProperties, true));
        try {
